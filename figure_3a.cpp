...
double boundary_condition(double a, std::vector<double> x)
{
	if (norme_2(x)>1)
	{
		return 0;
...
double integral(double(*f)(double alpha, double time), double a, double x0, double x1, int n)
{
	double step = (x1 - x0)/n;
	double area = 0.0;
	for(int i = 0; i < n; ++i)
...
double solution_gradient_lin_elliptic_sample( double mu, double a, double e, std::vector<double> x,double S)
{
	int ifault;
	...
	double dt=W/200/(1+0.5*sqrt(norme_2(x)));
	double t=0;
	while (t<W)
	{
		if (norme_2(y) > 1)
	...
	if (U < 0.5)
	{
		return 2*S*(sol_funct(a,y)+ (2*k00+a)*pow(norme_2(y),2)*(1-pow(norme_2(y),2))*pow(1-pow(norme_2(y),2), k00+a/2.0-1)) /gamma_pdf(W,mu) ;
	}
	else
	{
		return solution_gradient_lin_elliptic_sample(mu, a,e, y, 2*S*scalar_product(coeff_b0(1,x),weight_green(1,x,y,a)) /gamma_pdf(W,mu));
	}

}


double solution_gradient_lin_elliptic(double mu, double a, double e, std::vector<double> x, int n)
{
	double result = 0;
	double v;
	int m=1;
	for (int i=0; i <n;i++)
	{
		double trunc = max(-1000,min(1000,solution_gradient_lin_elliptic_sample(mu, a, e, x,1)));
		result += trunc;
	}
	printf("result=%f\n",result/n);
	return (result/n);
...
int main()
{
	...
	for (k=0;k<=20;k++)
	{
		...
		int MCSamples =10000000;

		double y = solution_gradient_lin_elliptic(mu,alpha,0,x,MCSamples);
		c2 = clock();
		printf("x\t\t y\n");
