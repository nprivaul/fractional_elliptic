...
double solution_gradient_lin_elliptic_sample( double mu, double a, double e, std::vector<double> x,double S)
{
	...
	double dt=W/50/(1+0.5*sqrt(norme_2(x)));
	double t=0;
	while (t<W)
	{
		if (norme_2(y) > 1)
...
double solution_gradient_lin_elliptique(double mu, double a, double e, std::vector<double> x, int n)
{
	double result = 0;
	double v;
	int m=1;
	for (int i=0; i <n;i++)
	{
		double truc = max(-1000,min(1000,solution_gradient_lin_elliptic_sample(mu, a, e, x,1)));
		result += truc;
	}
	printf("result=%f\n",result/n);
	return (result/n);
...
int main()
{
	...
	for (k=0;k<=20;k++)
	{
		...
		double y = solution_gradient_lin_elliptique(mu,alpha,0,x,20000000);
		c2 = clock();
		printf("x\t\t y\n");
